...
#define VAL_CONTRAST            (0x20)      //LCD contrast
...
int sb1602b_init(void)
{
    ...
    if (ret != 0) {
        printk("fail internal OSC frequency\n");
        return -1;
    }
    ...
    if (ret != 0) {
        printk("fail Power/ICON/Contrast Set\n");
        return -1;
    }
    ret = write_cmd(CTRL_BYTE_CMD, CMD_CNTRSET(VAL_CONTRAST_C3210));
    if (ret != 0) {
        printk("fail Constrast Set\n");
        return -1;
    }
...
static int write_lcd(uint8_t ctrl, uint8_t data, uint32_t usec)
{
    ...
    if (ret != 0) {
        printk("fail i2c_write_dt: addr=%02x [%02x]<--%02x (err=%d)\n", dev_i2c.addr, buf[0], buf[1], ret);
        return ret;
    }
    k_usleep(usec);
    return 0;
